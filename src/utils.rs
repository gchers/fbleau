...
pub fn load_data<T>(fname: &str) -> Result<(Array2<T>, Array1<Label>), Box<dyn Error>>
where
    T: FromStr,
{
    let mut reader = ReaderBuilder::new().has_headers(false).from_path(fname)?;
    ...
    for result in reader.records() {
        ...
        inputs.extend(
            record
                ...
                .map(|x| x.trim().parse::<T>().ok().expect("Failed to parse")),
        );
        targets.push(record[0].parse::<usize>().unwrap_or_else(|_| {
            panic!(format!(
    ...
    let inputs_a = if let Some(d) = ncols {
        let n = inputs.len() / d;
        Array::from(inputs).into_shape((n, d))?
    } else {
        panic!("File has wrong format");
    };
...
pub fn prepare_data(
    mut train_x: Array2<f64>,
    train_y: Array1<Label>,
    mut test_x: Array2<f64>,
    test_y: Array1<Label>,
    scale: bool,
) -> (
    Array2<f64>,
    ...
    let (train_y, mapping) =
        vectors_to_ids(train_y.view().into_shape((train_y.len(), 1)).unwrap(), None);
    let train_nlabels = mapping.len();
    // Remap test labels according to the mapping used for training labels.
    let (test_y, mapping) = vectors_to_ids(
        test_y.view().into_shape((test_y.len(), 1)).unwrap(),
        Some(mapping),
    );
...
/// unique ids.
pub fn vectors_to_ids(
    objects: ArrayView2<usize>,
    ...
    for x in objects.outer_iter() {
        let id = mapping.entry(x.to_owned()).or_insert_with(|| {
            next_id += 1;
            next_id - 1
        });
        out.push(*id);
...
mod tests {
    ...
    fn test_vectors_to_ids() {
        ...
        let b = array![[9, 0, 2], [1, 2, 3], [0, 0, 0], [1, 2, 5], [6, 4, 3]];
    ...
    #[test]
    fn test_scale() {
        let mut a = array![[2., 3., 5.], [1., 2., 10.], [0., 1., 2.]];

        scale01(&mut a);

        assert_eq!(a, array![[1., 1., 0.375], [0.5, 0.5, 1.], [0., 0., 0.]]);
    }
}
